...
public sealed class MovementTag : IBaseTag
{
	...
	public void Update(TaggedPlayer player)
	{
		...
		CurrentAxis = new(right - left, down - up);
		...
		if (CurrentAxis == Vector2.Zero)
		{
			if (UseLeftThumb && gamepadExists)
			{
				...
				CurrentAxis = new(Math.Abs(l.X) >= TaggedInput.DeadAxis.X ? l.X : 0, Math.Abs(l.Y) >= TaggedInput.DeadAxis.Y ? l.Y : 0);
			}
			else if (UseRightThumb && gamepadExists)
			{
				var r = Input.RightThumbStick(player.GamePad);
				CurrentAxis = new(Math.Abs(r.X) >= TaggedInput.DeadAxis.X ? r.X : 0, Math.Abs(r.Y) >= TaggedInput.DeadAxis.Y ? r.Y : 0);
			}
		}
	}
}
